...
function run(name, setup, env = {}) {
  ...
  // Archive-backed masking is opt-in; the context scenario needs it on.
  mkdirSync(join(home, ".config"), { recursive: true });
  writeFileSync(join(home, ".config/condensed-milk.json"), JSON.stringify({ archive: { enabled: true } }));
  setup({ home, cwd });
